...
#define FROM_STRING(STR)                       \
    ((STR != NULL) ? PyUnicode_FromString(STR) \
...
int _PyWideStringList_Copy(PyWideStringList *list, const PyWideStringList *list2)
{
    assert(_PyWideStringList_CheckConsistency(list));
...
#define FROM_WSTRING(STR)                            \
    ((STR != NULL) ? PyUnicode_FromWideChar(STR, -1) \
                   : (Py_INCREF(Py_None), Py_None))
...
config_read_complex_options(PyConfig *config)
{
    ...
    {
        if (config_get_env(config, "PYTHONFAULTHANDLER") || config_get_xoption(config, L"faulthandler"))
        {
            config->faulthandler = 1;
        }
    }
    if (config_get_env(config, "PYTHONPROFILEIMPORTTIME") || config_get_xoption(config, L"importtime"))
    {
                     ...
                     Py_ssize_t *opt_index)
{
    ...
    do
    {
        ...
        case 0:
            ...
            if (wcscmp(_PyOS_optarg, L"always") == 0 || wcscmp(_PyOS_optarg, L"never") == 0 || wcscmp(_PyOS_optarg, L"default") == 0)
            {
                status = PyConfig_SetString(config, &config->check_hash_pycs_mode,
                                            _PyOS_optarg);
                if (_PyStatus_EXCEPTION(status))
    ...
    if (config->run_command == NULL && config->run_module == NULL && _PyOS_optind < argv->length && wcscmp(argv->items[_PyOS_optind], L"-") != 0 && config->run_filename == NULL)
    {
...
PyConfig_Read(PyConfig *config)
{
    ...
    if (config->orig_argv.length == 0 && !(config->argv.length == 1 && wcscmp(config->argv.items[0], L"") == 0))
    {
